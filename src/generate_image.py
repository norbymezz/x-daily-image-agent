...
def get_daily_image(config: dict[str, Any]) -> Path:
    candidate = find_candidate_image(Path(config['source_dir']), config['allowed_formats'])
    return candidate if candidate else make_placeholder_image(Path(config['output_dir']), config)
